...
class VidaaTv(private val host: String) {
    ...
    private fun send(topic: String, payload: String): Boolean {
        return tryMqtt(false, topic, payload) || tryMqtt(true, topic, payload)
    }

    private fun tryMqtt(tls: Boolean, topic: String, payload: String): Boolean {
        return try {
            ...
            val ack = ByteArray(4)
            val read = inp.read(ack)
            if (read < 2 || ack[0] != 0x20.toByte()) { socket.close(); return false }
    ...
    private fun publishPacket(topic: String, msg: String): ByteArray {
        ...
        val len = t.size + m.size
        return byteArrayOf(0x30) + varLen(len) + t + m
    }
